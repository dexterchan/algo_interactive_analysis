...
def _initialize_price_feature_instance(nt: NamedTuple, price:pd.Series) -> Feature:
    if isinstance(nt, Log_Price_Feature_Interface):
        return Log_Price_Feature(**nt, df_price=price)
    elif isinstance(nt, SMA_Cross_Feature_Interface):
        return SMA_Cross_Feature(**nt, df_price=price)
    elif isinstance(nt, RSI_Feature_Interface):
        return RSI_Feature(**nt,df_price=price)
    else:
        raise NotImplementedError(f"Not supporting this config: {nt}")

def create_feature_from_close_price(ohlcv_candles:pd.DataFrame, feature_pools:list[Feature_Definition]) -> np.ndarray:
    ...
    feature_set = [f[-shortest_feature_length:].reshape(shortest_feature_length,-1) for f in feature_set]
